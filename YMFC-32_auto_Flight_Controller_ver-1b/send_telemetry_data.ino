...
void send_telemetry_data(void) {
  ...
  if (telemetry_loop_counter == 1){
   /* Serial.println(telemetry_loop_counter);
    Serial.println("1");*/
  ...
  if (telemetry_loop_counter <= 34) {
   /* if (telemetry_loop_counter == 1){
    Serial.println("J :");
    Serial.println(telemetry_send_byte);
    Serial.println(0b1 << 16);
    Serial.println(0b1 << 0);
    }*/
    check_byte ^= telemetry_send_byte;
    GPIOB_BASE->BSRR = 0b1 << 16;                                                             //Reset output PB0 to 0 to create a start bit.
    delayMicroseconds(100);                                                                  //Delay 104us (1s/9600bps)
    for (telemetry_bit_counter = 0; telemetry_bit_counter < 8; telemetry_bit_counter ++) {    //Create a loop fore every bit in the
    /*  Serial.println(telemetry_bit_counter);
      Serial.println(telemetry_send_byte >> telemetry_bit_counter & 0b1);*/
     ...
     /* if(telemetry_bit_counter % 2 == 0) GPIOB_BASE->BSRR = 0b1 << 16;
      else GPIOB_BASE->BSRR = 0b1 << 0;*/
      delayMicroseconds(100);                                                                 //Delay 104us (1s/9600bps)
    }
    //Send a stop bit
